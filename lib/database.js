...
export const peopleService = {
  ...
  async getAll() {
    try {
      ...
      console.log('Successfully fetched people:', data?.length || 0, 'records')
  ...
  async create(personData) {
    try {
      console.log('Creating person in database:', personData)
      const { data, error } = await supabase
        .from('people')
        .insert([personData])
        .select()
  ...
  async createBatch(peopleData) {
    try {
      console.log('Creating batch of people in database:', peopleData.length, 'records')
      const { data, error } = await supabase
        .from('people')
        .insert(peopleData)
        .select()
      ...
      console.log('Successfully created batch:', data?.length || 0, 'records')
      return data ? data.map(transformPersonFromDB) : []
    } catch (error) {
